	  	...
	  	<tr valign="top">
	    	<td height="30">
		  		<table width="100%" border="0" cellpadding="0" cellspacing="10" class="main_tab">
		    		<tr>
			  			<td class="fftd">
			  				<form id="searchForm" action="EmployeeListServlet" method="post">
			  					<input id="currentPage" style="display: none;" type="text" name="currentPage" />
					    		<table width="100%" border="0" cellpadding="0" cellspacing="0">
						  			<tr>
						    			<td class="font3">
						    				职位：<select name="job_id" style="width:143px;">
						    					<option value="0">--请选择职位--</option>
					    						<c:forEach items="${requestScope.jobs }" var="job">
					    							<option <c:if test="${job.id==param.job_id}">selected</c:if> value="${job.id}">${job.name}</option>
					    						</c:forEach>
						    				</select>
						    				姓名：<input type="text" name="name" value="${param.name}">
						    				身份证号码：<input type="text" name="cardId" maxlength="18" value="${param.cardid}">
						    			</td>
						  			</tr>
						  			<tr>
						    			<td class="font3">
						    				性别：<select name="sex" style="width:143px;">
								    			...
								    			<option <c:if test="${param.sex=='1'}">selected</c:if> value="1">男</option>
								    			<option <c:if test="${param.sex=='2'}">selected</c:if> value="2">女</option>
								    		</select>
						    				手机：<input type="text" name="phone" value="${param.phone}">
						    				所属部门：<select  name="dept_id" style="width:100px;">
								   				<option value="0">--部门选择--</option>
									   			<c:forEach items="${requestScope.depts }" var="dept">
						    						<option <c:if test="${dept.id==param.dept_id}">selected</c:if> value="${dept.id}">${dept.name}</option>
						    					</c:forEach>
											</select>&nbsp;
